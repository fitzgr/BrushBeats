...
- Treat IndexedDB as the long-term source of truth. Cookies should become a short-lived import source only.
- Introduce stable generated IDs and sync metadata now so local models can mirror future backend tables cleanly.
- Keep migrations explicit, versioned, and idempotent so failed upgrades can be retried safely.
- Separate data services from React UI so storage, gamification rules, and sync logic remain testable and portable.
...
- Design a database versioning strategy
  ...
  - Document what qualifies as a breaking schema change versus a data backfill.
- Define the initial schema and object stores
  ...
  - Add sync-friendly metadata fields now: createdAt, updatedAt, syncVersion, deleted, and deletedAt where relevant.
...
- Add infrastructure guardrails early
  ...
  - Define upgrade test fixtures before more data models are added.
